"""Launch Pix2Pix training as a SageMaker PyTorch Training Job.

Usage:
    python training/pix2pix/run_job.py \
        --env dev \
        --epochs 200 \
...
def run(args: argparse.Namespace) -> None:
    ...
    output_s3    = f"s3://{ARTIFACTS_BUCKET}/models/pix2pix/{args.env}/"
    ...
    estimator = PyTorch(
        ...
        base_job_name="pixelsense-pix2pix",
        sagemaker_session=sm_session,
    )

    log.info("Launching Pix2Pix SageMaker Training Job")
    log.info("  instance type  : %s", args.instance_type)
    log.info("  processed data : %s", processed_s3)
    log.info("  output path    : %s", output_s3)

    estimator.fit(inputs={"train": processed_s3}, wait=args.wait,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Launch Pix2Pix SageMaker Training Job")
